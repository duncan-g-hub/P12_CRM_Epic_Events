...
def test_create_success_as_commercial(runner, mocker, commercial_ctx, payload_commercial, fake_customer):
...
def test_update_no_customers_available(runner, mocker, commercial_ctx, payload_commercial):
    ...
    mocker.patch(
        ...
        side_effect=ValueError("Aucun client disponible."),
    )
...
def test_update_as_commercial(runner, mocker, commercial_ctx, payload_commercial, fake_customers_list, fake_customer):
    ...
    mocker.patch(
        "commands.customers.get_customers",
        return_value=fake_customers_list,
    )
    ...
    user_input = "\n".join([
        ...
        "",  # nouvelle entreprise (ignoré)
        ""
    ])

    result = runner.invoke(
        customers_group, ["update"],
        input=user_input, obj=commercial_ctx,
    )
...
def test_update_as_gestion_reassigns_commercial(runner, mocker, gestion_ctx, payload_gestion, fake_customers_list,
    ...
    mocker.patch(
        "commands.customers.get_commercials",
        return_value=(" nom : client (id : 1)", ["1"]),
    )
    mock_reassign = mocker.patch(
        "commands.customers.update_customer_commercial",
        return_value=fake_customer,
    )

    user_input = "\n".join([
        "1",  # id du client
        "1",  # nouveau commercial
    ])
...
def test_update_as_gestion_no_commercials_available(runner, mocker, gestion_ctx, payload_gestion, fake_customers_list):
    ...
    mocker.patch(
        "commands.customers.get_commercials",
        side_effect=ValueError("Il n'éxiste aucun commercial."),
    )
    result = runner.invoke(
        customers_group,
        ["update"],
...
def test_update_as_gestion_displays_permission_error(runner, mocker, gestion_ctx, payload_gestion, fake_customers_list
    ...
    mocker.patch(
        "commands.customers.get_commercials",
        return_value=(" id : 1 - Nom : commercial", ["1"]),
    )
    mocker.patch(
        "commands.customers.update_customer_commercial",
        side_effect=PermissionError("Action non autorisée."),
...
def test_display_no_customers_available(runner, mocker, gestion_ctx, payload_gestion):
    ...
    mocker.patch(
        "commands.customers.get_customers",
        side_effect=ValueError("Clients introuvables."),
    )

...
def test_create_success_as_gestion(runner, mocker, gestion_ctx, payload_gestion, fake_gestion):
    ...
    mock_create = mocker.patch("commands.collaborators.create_collaborator", return_value=fake_gestion)

    user_input = "\n".join(["Nouveau Collab", "nouv.collab@crm.com", "Mdp12345!", "Mdp12345!", "0123456789", "3"])
    result = runner.invoke(collaborators_group, ["create"], input=user_input, obj=gestion_ctx)
...
def test_create_denied_for_wrong_role(runner, mocker, commercial_ctx, payload_commercial):
    ...
    result = runner.invoke(collaborators_group, ["create"], input="", obj=commercial_ctx)
...
def test_create_password_mismatch_then_retry(runner, mocker, gestion_ctx, payload_gestion, fake_gestion):
    mocker.patch("commands.utils.decode_token", return_value=payload_gestion)
    mocker.patch("commands.collaborators.create_collaborator", return_value=fake_gestion)

    user_input = "\n".join(["test", "test@email.com", "Mdp12345", "Mdp54321", "Mdp12345!", "Mdp12345!",
                            "0123456789", "3", ""])
    result = runner.invoke(collaborators_group, ["create"],input=user_input, obj=gestion_ctx)
...
def test_create_displays_permission_error(runner, mocker, gestion_ctx, payload_gestion):
    mocker.patch("commands.utils.decode_token", return_value=payload_gestion)
    mocker.patch("commands.collaborators.create_collaborator", side_effect=PermissionError("Action non autorisée."))
...
def test_update_success(runner, mocker, gestion_ctx, payload_gestion, fake_gestion, fake_collaborators_list):
    mocker.patch("commands.utils.decode_token", return_value=payload_gestion)
    mocker.patch("commands.collaborators.get_collaborators", return_value=fake_collaborators_list)
    mock_update = mocker.patch("commands.collaborators.update_collaborator", return_value=fake_gestion)

    user_input = "\n".join(["1", "", "", "", "", "", ""])
    result = runner.invoke(collaborators_group, ["update"], input=user_input, obj=gestion_ctx)
...
def test_update_denied_for_wrong_role(runner, mocker, commercial_ctx, payload_commercial):
    ...
    result = runner.invoke(collaborators_group, ["update"], input="", obj=commercial_ctx)
...
def test_update_displays_permission_error(runner, mocker, gestion_ctx, payload_gestion,
    ...
    mocker.patch("commands.utils.decode_token", return_value=payload_gestion)
    mocker.patch("commands.collaborators.get_collaborators", return_value=fake_collaborators_list)
    mocker.patch("commands.collaborators.update_collaborator", side_effect=PermissionError("Action non autorisée."))
...
def test_display_success(runner, mocker, commercial_ctx, payload_commercial, fake_collaborators_list):
    ...
    user_input = "\n".join(["", "1"])
    result = runner.invoke(collaborators_group, ["display"], input=user_input, obj=commercial_ctx)
...
def test_display_with_role_filter(runner, mocker, gestion_ctx, payload_gestion, fake_collaborators_list):
    mocker.patch("commands.utils.decode_token", return_value=payload_gestion)
    mock_get = mocker.patch("commands.collaborators.get_collaborators", return_value=fake_collaborators_list)
    mocker.patch("commands.collaborators.display_collaborator")

    user_input = "\n".join(["1", "1"])
    result = runner.invoke(collaborators_group, ["display"], input=user_input, obj=gestion_ctx)
